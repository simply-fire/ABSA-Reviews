...
class DatasetLoader:
        ...
        for text in df[col]:
            if text != "[]" and isinstance(text, str):
                ...
                for idx, pair in enumerate(text):
                    ...
                    if idx % 2 == 0:
                        reconstructed_dict = {}
                        reconstructed_dict[pair.split(splitter)[0].replace("'", "")] = (
                            pair.split(splitter)[1].replace("'", "")
                        )
                    else:
                        reconstructed_dict[pair.split(splitter)[0].replace("'", "")] = (
                            pair.split(splitter)[1].replace("'", "")
    ...
    def extract_rowwise_aspect_opinions(
        self, df, aspect_col, opinion_col, key, min_val=None
    ):
        """
        Create duplicate records based on number of aspect term labels in the dataset.
        Extract each aspect term for each row for reviews with muliple aspect term entries.
        ...
        df = df.loc[df.index.repeat(df["len"])]
        df["record_idx"] = df.groupby(df.index).cumcount()
        df["aspect"] = df[[aspect_col, "record_idx"]].apply(
            lambda x: x[0][x[1]][key] if len(x[0]) != 0 else "", axis=1
        )
        df["opinion_term"] = df[[opinion_col, "record_idx"]].apply(
            lambda x: x[0][x[1]][key] if len(x[0]) != 0 else "", axis=1
        )
    ...
    def create_data_in_ate_format(
        self, df, key, text_col, aspect_col, bos_instruction="", eos_instruction=""
    ):
        """
    ...
    def create_data_in_atsc_format(
        self,
        df,
        on,
        key,
    ...
    def create_data_in_aspe_format(
        self,
        df,
        key,
        label_key,
        ...
        df["labels"] = df[aspect_col].apply(
            lambda x: ", ".join([f"{i[key]}:{i[label_key]}" for i in x])
        )
        df["text"] = df[text_col].apply(lambda x: bos_instruction + x + eos_instruction)
        return df

    def create_data_in_aooe_format(
        self,
        df,
        aspect_col,
        opinion_col,
        ...
        if df is None:
            return
        df = self.extract_rowwise_aspect_opinions(
            df, aspect_col=aspect_col, opinion_col=opinion_col, key=key, min_val=1
        )
        df["text"] = df[[text_col, "aspect"]].apply(
            lambda x: bos_instruction
            + x[0]
            + delim_instruction
            + x[1]
    ...
    def create_data_in_aope_format(
        self,
        df,
        key,
        text_col,
        ...
        Prepare the data in the input format required.
        """
        df["labels"] = df[[aspect_col, opinion_col]].apply(
            lambda x: ", ".join(
                [f"{' '.join(i[key])}:{' '.join(j[key])}" for i, j in zip(x[0], x[1])]
            ),
            axis=1,
        )
        df["text"] = df[text_col].apply(lambda x: bos_instruction + x + eos_instruction)
    ...
    def create_data_in_aoste_format(
        self,
        df,
        key,
        label_key,
        ...
        df["labels"] = df[[aspect_col, opinion_col]].apply(
            lambda x: ", ".join(
                [
                    f"{' '.join(i[key])}:{' '.join(j[key])}:{label_map[i[label_key]]}"
