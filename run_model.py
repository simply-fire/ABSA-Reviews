...
print(
    ...
    (
        "training"
        if config.mode == "train"
        else ("inference" if config.mode == "eval" else "Individual sample inference")
    ),
)
...
training_args = {
    ...
    "evaluation_strategy": (
        config.evaluation_strategy if config.id_te_data_path is not None else "no"
    ),
    ...
    "num_train_epochs": (
        config.num_train_epochs if config.num_train_epochs is not None else None
    ),
...
if config.mode != "cli":
    ...
    if config.task == "ate":
        ...
        if loader.train_df_ood is not None:
            loader.train_df_ood = loader.create_data_in_atsc_format(
                loader.train_df_ood,
                "aspectTerms",
                "term",
                "raw_text",
        ...
        if loader.test_df_ood is not None:
            loader.test_df_ood = loader.create_data_in_atsc_format(
                loader.test_df_ood,
                "aspectTerms",
                "term",
                "raw_text",
        ...
        if loader.train_df_id is not None:
            loader.train_df_id = loader.create_data_in_joint_task_format(
                loader.train_df_id,
                "term",
                "polarity",
                "raw_text",
        ...
        if loader.train_df_ood is not None:
            loader.train_df_ood = loader.create_data_in_joint_task_format(
                loader.train_df_ood,
                "term",
                "polarity",
                "raw_text",
        ...
        if loader.test_df_ood is not None:
            loader.test_df_ood = loader.create_data_in_joint_task_format(
                loader.test_df_ood,
                "term",
                "polarity",
                "raw_text",
    ...
    # Tokenize dataset
    id_ds, id_tokenized_ds, ood_ds, ood_tokenized_ds = (
        loader.set_data_for_training_semeval(t5_exp.tokenize_function_inputs)
    )
    ...
    if config.mode == "train":
        ...
        if id_tokenized_ds.get("train") is not None:
            ...
            id_tr_df.to_csv(
                os.path.join(
                    config.output_path, f"{config.experiment_name}_id_train.csv"
                ),
                index=False,
            )
            print("*****Train Metrics*****")
            precision, recall, f1, accuracy = t5_exp.get_metrics(
                id_tr_df["labels"], id_tr_pred_labels
            )
        ...
        if id_tokenized_ds.get("test") is not None:
            ...
            id_te_df.to_csv(
                os.path.join(
                    config.output_path, f"{config.experiment_name}_id_test.csv"
                ),
                index=False,
            )
            print("*****Test Metrics*****")
            precision, recall, f1, accuracy = t5_exp.get_metrics(
                id_te_df["labels"], id_te_pred_labels
            )
            print("Precision: ", precision)
            print("Recall: ", recall)
            print("F1-Score: ", f1)
        ...
        if ood_tokenized_ds.get("train") is not None:
            ood_tr_pred_labels = t5_exp.get_labels(
                tokenized_dataset=ood_tokenized_ds,
                sample_set="train",
                batch_size=config.per_device_eval_batch_size,
                max_length=config.max_token_length,
            )
            ...
            ood_tr_df.to_csv(
                os.path.join(
                    config.output_path, f"{config.experiment_name}_ood_train.csv"
                ),
                index=False,
            )
            print("*****Train Metrics - OOD*****")
            precision, recall, f1, accuracy = t5_exp.get_metrics(
                ood_tr_df["labels"], ood_tr_pred_labels
            )
            print("Precision: ", precision)
        ...
        if ood_tokenized_ds.get("test") is not None:
            ood_te_pred_labels = t5_exp.get_labels(
                tokenized_dataset=ood_tokenized_ds,
                sample_set="test",
                batch_size=config.per_device_eval_batch_size,
                max_length=config.max_token_length,
            ...
            ood_te_df.to_csv(
                os.path.join(
                    config.output_path, f"{config.experiment_name}_ood_test.csv"
                ),
                index=False,
            )
            print("*****Test Metrics - OOD*****")
            precision, recall, f1, accuracy = t5_exp.get_metrics(
                ood_te_df["labels"], ood_te_pred_labels
            )
            print("Precision: ", precision)
            print("Recall: ", precision)
            print("F1-Score: ", precision)
    ...
    main = []
    file_path = config.test_input
    with open(file_path, "r") as file:
        statements = file.read().splitlines()
        for statement in statements:
            print(statement)
            model_input = bos_instruction_id + statement + eos_instruction
            input_ids = t5_exp.tokenizer(model_input, return_tensors="pt").input_ids
            outputs = t5_exp.model.generate(
                input_ids, max_length=config.max_token_length
            )
            print(
                "Model output: ",
                t5_exp.tokenizer.decode(outputs[0], skip_special_tokens=True),
            )
            s = t5_exp.tokenizer.decode(outputs[0], skip_special_tokens=True)
            x = s.split(",")
            for i in range(len(x)):
                main.append({x[i].split(":")[0]: x[i].split(":")[1]})
    ...
    def print_sentiment_counts(counts):
        for key, value in counts.items():
            print(f"Key: {key}")
            print(f"\tPositive: {value['positive']}")
            print(f"\tNegative: {value['negative']}")
            print(f"\tNone: {value['none']}")
            print()

    print(main)
    print_sentiment_counts(count_sentiment_values(main))


# python run_model.py -mode cli -task joint -model_checkpoint kevinscaria/joint_tk-instruct-base-def-pos-neg-neut-combined -test_input 'input_file.txt'
